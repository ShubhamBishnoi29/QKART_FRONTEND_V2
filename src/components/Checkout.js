...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      //updating available addresses
      setAddresses({ ...addresses, all: res.data });
      //resetting new address
      setNewAddress((currNewAddress) => ({
        ...currNewAddress,
        isAddingNewAddress: false,
        value: "",
      }));
      console.log("--POST address (addAddress)  data------>", res.data);
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      //updating available addresses
      setAddresses({ ...addresses, all: res.data }); 
      // console.log("--DELETE address (deleteAddress)  data------>", res.data);
    } catch (e) {
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
  ...
  const performCheckout = async (token, items, addresses) => {
    let checkRequest = validateRequest(items, addresses);
    if (checkRequest) {
      try {
        ...
        let balance = localStorage.getItem("balance");
        let totalCartValue = getTotalCartValue(items);
        let balanceLeft = balance - totalCartValue;
        localStorage.setItem('balance', balanceLeft);
        // Redirect customer to the Thanks page
        history.push("/thanks");
      } catch (e) {
        if (e.response) {
          enqueueSnackbar(e.response.data.message, { variant: "error" });
    ...
    // console.log("addresses id===>", addresses.selected);
    // console.log("addresses arr===>", addresses.all);
  ...
  // Fetch list of addresses for a user on page load
  useEffect(() => {
  ...
  //Handel the selected address
  const handleClick = (id) => {
    setSelected(id);
  };


// curl 'http://localhost:8082/api/v1/user/addresses' -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImNyaW8uZG8iLCJpYXQiOjE2NTg4NDA1NDUsImV4cCI6MTY1ODg2MjE0NX0.PPZEgPr2veBTvlmMyfc4QH3-Dl9ql9mW-is1Mrna6Go'

// curl 'http://localhost:8082/api/v1/user/addresses' -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImNyaW8uZG8iLCJpYXQiOjE2NTg4NDA1NDUsImV4cCI6MTY1ODg2MjE0NX0.PPZEgPr2veBTvlmMyfc4QH3-Dl9ql9mW-is1Mrna6Go' -H 'Content-Type: application/json' --data-raw '{"address":"Test address\n12th street, Mumbai"}'

// curl 'http://localhost:8082/api/v1/user/addresses/lCS83sdUePplA4nJf7UZP' -X 'DELETE' -H 'Accept: application/json, text/plain, */*' -H 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImNyaW8uZG8iLCJpYXQiOjE2NTg4NDA1NDUsImV4cCI6MTY1ODg2MjE0NX0.PPZEgPr2veBTvlmMyfc4QH3-Dl9ql9mW-is1Mrna6Go'
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              ...
              {addresses.all.length != 0 ?
                (addresses.all.map((value) => (
                  <Box className={selected == value._id ? "selected address-item " : "not-selected address-item "}
                    key={value._id}
                    onClick={() => {
                      handleClick(value._id)
                      setAddresses({ ...addresses, selected: value._id});
                    }}
                  >
                    <Typography my="1rem">
                      {value.address}
                    </Typography>
                    <Button variant="text" startIcon={<Delete />} onClick={() => deleteAddress(token, value._id)}>
                      Delete
                    </Button>
                  </Box>))
                ) :
